...
const packages = [
  {
    ...
    items: ["3 matched jobs", "CV tailored to 3 roles", "3 cover letters"],
  },
  {
    key: "professional",
    icon: "💼",
...
function handleGoogleFormSubmission(payload) {
  ...
  const url = GOOGLE_FORM_URL.includes("?")
  ...
  return (
    <label className="block">
      ...
      {React.cloneElement(children, {
        className:
          "h-11 w-full rounded-xl border border-slate-200 bg-white px-3.5 text-sm font-medium text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10",
      })}
    </label>
  );
}
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main id="home">
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-5 pb-20 pt-16 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:px-10 lg:pb-28 lg:pt-24">
            <div>
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-blue-100 bg-blue-50 px-4 py-2 text-xs font-black uppercase tracking-wider text-blue-700">
                <Globe2 size={14} /> Personalized job search
              </div>
            ...
            <div className="relative">
              ...
              <div className="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-950 p-6 text-white shadow-2xl sm:p-8">
                ...
                <div className="mt-8 space-y-3">
                  {[
                    ["Target role", "Software Engineer", BriefcaseBusiness],
                    ["Location", "Canada · Remote · UK", MapPin],
                    ["Experience", "3+ years", Clock3],
        ...
        <section id="who" className="scroll-mt-24 bg-slate-950 text-white">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 lg:grid-cols-2 lg:items-center lg:px-10 lg:py-24">
            <div>
              ...
              <p className="mt-5 max-w-xl leading-7 text-slate-300">
                Whether you're starting out, moving up, changing industries, or
                targeting international opportunities, your search begins with
          ...
          <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10 lg:py-24">
            ...
            <div className="mt-12 grid gap-4 lg:grid-cols-5">
              {packages.map((pkg) => (
                <button
                  key={pkg.key}
                  type="button"
                  onClick={() => set("careerLevel", pkg.key)}
            ...
            {form.careerLevel && (
              <div className="mt-7 rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4 text-sm font-bold text-blue-900">
                Selected:{" "}
                {packages.find((x) => x.key === form.careerLevel)?.name} — $
                {packages.find((x) => x.key === form.careerLevel)?.price}
        ...
        <section id="match" className="scroll-mt-24 bg-white">
          <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10 lg:py-24">
                    ...
                    <Field label="CV (optional)">
                      <label className="flex h-11 cursor-pointer items-center gap-3 rounded-xl border border-dashed border-slate-300 bg-white px-3 text-sm font-semibold text-slate-500 hover:border-blue-400 hover:text-blue-600">
                        ...
                        <span className="truncate">
                          {form.cv ? form.cv.name : "Upload PDF, DOC or DOCX"}
                        </span>
                        <input
                          type="file"
                          accept=".pdf,.doc,.docx"
                  ...
                  <div className="mt-7 flex flex-col gap-4 border-t border-slate-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    >
                      {busy
                        ? "Submitting..."
                        : form.careerLevel
                          ? `Get started — $${packages.find((x) => x.key === form.careerLevel)?.price}`
                          : "Find my jobs"}
        ...
        <section className="border-t border-slate-200 bg-slate-50">
          <div className="mx-auto flex max-w-7xl flex-col gap-5 px-5 py-14 sm:px-8 md:flex-row md:items-center md:justify-between lg:px-10">
            <div>
              <h2 className="text-xl font-black">Ready to search smarter?</h2>
